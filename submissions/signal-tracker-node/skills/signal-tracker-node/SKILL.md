---
name: signal-tracker-node
description: "Track smart money wallet activity and aggregated buy signals (node implementation)"
version: "1.0.0"
author: "Mingtao"
tags:
  - smart-money
...
signal-tracker-node 让 AI Agent 能够追踪聪明钱钱包的交易动态，并获取聚合买入信号。
它提供 `npx @ganlinux/signal-tracker-node` CLI 工具用于格式化展示，所有链上数据查询通过 `onchainos` CLI 完成。
...
2. Node.js >= 18 已安装
3. 通过 `plugin-store install signal-tracker-node` 安装或直接使用 `npx @ganlinux/signal-tracker-node`
4. 网络连接正常
...
```bash
npx @ganlinux/signal-tracker-node track --address <WALLET_ADDRESS> --chain ethereum
```
...
```bash
npx @ganlinux/signal-tracker-node signals --chain ethereum
```
...
```bash
npx @ganlinux/signal-tracker-node price --address <TOKEN_ADDRESS> --chain ethereum
```
